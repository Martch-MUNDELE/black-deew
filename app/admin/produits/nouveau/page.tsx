                ...
                {vt.options.map((opt, oIdx) => (
                  <div key={oIdx} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '10px 12px', borderRadius: 10, background: 'rgba(245,200,66,0.06)', border: '1px solid rgba(245,200,66,0.18)' }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <input type="number" inputMode="decimal" placeholder="Prix spécifique (optionnel)" value={vt.prices?.[opt] ?? ''} onChange={e => updateOptionPrice(vIdx, opt, e.target.value === '' ? null : parseFloat(e.target.value))} style={{ width: '100%', padding: '7px 10px', borderRadius: 8, border: '1px solid rgba(245,200,66,0.3)', background: 'rgba(0,0,0,0.3)', color: '#F5C842', fontSize: 13, outline: 'none', fontFamily: 'DM Sans, sans-serif', boxSizing: 'border-box' as const, appearance: 'none' as any, MozAppearance: 'textfield' as any, WebkitAppearance: 'none' as any }} />
