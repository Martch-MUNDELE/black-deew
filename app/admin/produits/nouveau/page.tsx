...
export default function NouveauProduit() {
  ...
  const addVariantType = () => { setVariants(v => [...v, { type: '', options: [] }]); setNewOptionInputs(i => [...i, '']) }
  const updateVariantType = (idx: number, val: string) => { setVariants(v => v.map((vt, i) => i === idx ? { ...vt, type: val } : vt)) }
  const removeVariant = (idx: number) => { setVariants(v => v.filter((_, i) => i !== idx)); setNewOptionInputs(i => i.filter((_, j) => j !== idx)) }
  const addOption = (idx: number) => { const val = newOptionInputs[idx]?.trim(); if (!val) return; setVariants(v => v.map((vt, i) => i === idx ? { ...vt, options: [...vt.options, val] } : vt)); setNewOptionInputs(i => i.map((v, j) => j === idx ? '' : v)) }
  const removeOption = (vIdx: number, oIdx: number) => { setVariants(v => v.map((vt, i) => i === vIdx ? { ...vt, options: vt.options.filter((_, j) => j !== oIdx) } : vt)) }
  const updateOptionInput = (idx: number, val: string) => { setNewOptionInputs(i => i.map((v, j) => j === idx ? val : v)) }
  const updateOptionPrice = (vIdx: number, opt: string, price: number | null) => {
    setVariants(v => v.map((vt, i) => {
      if (i !== vIdx) return vt
      const prices = { ...(vt.prices || {}) }
      if (price === null || isNaN(price as number)) { delete prices[opt] } else { prices[opt] = price as number }
      return { ...vt, prices }
    }))
  }
  ...
  return (
    <div style={{ maxWidth: 560, margin: '0 auto', padding: '0 0 60px' }}>
      ...
      <div style={{ background: '#131009', border: '1px solid rgba(232,160,32,0.12)', borderRadius: 16, padding: '24px 20px', display: 'flex', flexDirection: 'column', gap: 16 }}>
        {([{ key: 'name', label: 'Nom', type: 'text' }, { key: 'description', label: 'Description', type: 'text' }, { key: 'ingredients', label: 'Ingrédients', type: 'text' }, { key: 'price', label: 'Prix (' + currency + ')', type: 'number' }] as Array<{key:string;label:string;type:string}>).map(f => (
          <div key={f.key}>
        ...
        <div><label style={labelStyle}>Image</label><ImageUpload imageUrl={form.image_url} onUpload={url => setForm(f => ({ ...f, image_url: url }))} /></div>
        <div>
          <label style={labelStyle}>Sous-catégorie</label>
          <select value={form.is_vip ? 'vip' : form.subcategory} onChange={e => { if (e.target.value === 'vip') { setForm(f => ({ ...f, is_vip: true })) } else { setForm(f => ({ ...f, is_vip: false, subcategory: e.target.value })) } }} style={{ width: '100%', padding: '13px 14px', borderRadius: 12, border: '1px solid rgba(232,160,32,0.2)', background: '#0E0B06', color: '#F5EDD6', fontSize: 14, outline: 'none', fontFamily: 'DM Sans, sans-serif', boxSizing: 'border-box' as const, cursor: 'pointer' }}>
        ...
        <div style={{ background: 'rgba(245,200,66,0.04)', border: '1px solid rgba(245,200,66,0.15)', borderRadius: 12, padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div style={{ fontSize: 13, fontWeight: 700, color: '#F5EDD6' }}>Variantes {variants.length > 0 ? '(' + variants.length + ')' : ''}</div>
            </div>
            {variantsOpen && <button onClick={addVariantType} style={{ padding: '7px 14px', borderRadius: 20, border: '1px solid rgba(245,200,66,0.4)', background: 'rgba(245,200,66,0.08)', color: '#F5C842', cursor: 'pointer', fontSize: 12, fontWeight: 700 }}>+ Ajouter</button>}
          </div>
          {variantsOpen && variants.map((vt, vIdx) => (
            <div key={vIdx} style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 10, padding: '12px 14px', border: '1px solid rgba(245,200,66,0.1)', display: 'flex', flexDirection: 'column', gap: 10 }}>
              <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                <input placeholder="Type (ex: Taille)" value={vt.type} onChange={e => updateVariantType(vIdx, e.target.value)} style={{ ...inputStyle, fontSize: 13, padding: '9px 12px' }} />
                ...
                {vt.options.map((opt, oIdx) => (
                  <div key={oIdx} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '10px 12px', borderRadius: 10, background: 'rgba(245,200,66,0.06)', border: '1px solid rgba(245,200,66,0.18)' }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 12, fontWeight: 700, color: '#F5C842', marginBottom: 6 }}>{opt}</div>
                      <input type="number" inputMode="decimal" placeholder="Prix spécifique (optionnel)" value={vt.prices?.[opt] ?? ''} onChange={e => updateOptionPrice(vIdx, opt, e.target.value === '' ? null : parseFloat(e.target.value))} style={{ width: '100%', padding: '7px 10px', borderRadius: 8, border: '1px solid rgba(245,200,66,0.3)', background: 'rgba(0,0,0,0.3)', color: '#F5C842', fontSize: 13, outline: 'none', boxSizing: 'border-box' as const }} />
                      <div style={{ fontSize: 10, color: '#7A6E58', marginTop: 4 }}>Laisser vide = prix principal</div>
                    </div>
        ...
        <div style={{ background: 'rgba(255,80,80,0.04)', border: '1px solid rgba(255,80,80,0.15)', borderRadius: 12, padding: '14px 16px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div><div style={{ fontSize: 13, fontWeight: 700, color: '#F5EDD6' }}>Promotion</div><div style={{ fontSize: 11, color: '#7A6E58', marginTop: 2 }}>Réduction sur ce produit</div></div>
          ...
          {promoOn && (
            <div style={{ marginTop: 14, display: 'flex', flexDirection: 'column', gap: 10 }}>
              <div><label style={labelStyle}>Réduction (%)</label><input type="number" min={5} max={80} value={form.discount ?? ''} onChange={e => { const v = parseInt(e.target.value); setForm(f => ({ ...f, discount: isNaN(v) ? 5 : Math.min(80, Math.max(5, v)) })) }} style={inputStyle} /></div>
              {discountedPreview !== null && savingsPreview !== null && (
                <div style={{ padding: '10px 14px', background: 'rgba(255,80,80,0.06)', borderRadius: 8, border: '1px solid rgba(255,80,80,0.12)' }}>
                  <div style={{ fontSize: 12, color: '#FF8080', fontWeight: 600 }}>Prix remisé : {discountedPreview} {currency}</div>
                  <div style={{ fontSize: 12, color: '#7A6E58', marginTop: 4 }}>Économie : {savingsPreview} {currency}</div>
                </div>
              )}
            </div>
          )}
