...
type OrderNotificationPayload = {
  ...
  vipItems?: NotificationItem[] | null
...
export async function sendOrderNotification(
  ...
  const standardItems = orderItems.filter((item) => !item.isVip)

  const itemsHtml = standardItems.map((item) => `
    <tr>
      <td style="padding:12px 16px;border-bottom:1px solid rgba(245,200,66,0.1);color:#F5EDD6;font-size:14px">${item.quantity}× ${item.product_name}</td>
      <td style="padding:12px 16px;border-bottom:1px solid rgba(245,200,66,0.1);color:#F5C842;font-size:14px;font-weight:700;text-align:right">${(item.quantity * item.unit_price).toFixed(2)} ${currency}</td>
    </tr>`
  ).join('')

  const deliveryFee = order.delivery_fee ?? 0
  const standardTotal = standardItems.reduce((sum, item) => sum + item.unit_price * item.quantity, 0)
  ...
  const toEmail = adminEmail || process.env.RELAY_EMAIL || 'heupel.martial@gmail.com'
